...
class AllowTaskReparentingActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContent {
            Column(Modifier.systemGesturesPadding().fillMaxSize()) {
                Text("${this@AllowTaskReparentingActivity.javaClass.simpleName}")
                val activityName = "OtherInstanceActivity"
                Button({
                    Intent().apply {
                        val packageName = this@AllowTaskReparentingActivity.packageName
